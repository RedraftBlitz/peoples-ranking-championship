...
export async function POST(
  ...
  if (snapshot.status !== "pending_review" || !review.ready) {
    return Response.json(
      { error: "Resolve every blocking identity issue before approval." },
      { status: 409 },
    );
  }
  ...
  await db.batch([
    db
      .prepare("UPDATE market_snapshots SET status = 'superseded' WHERE status = 'approved' AND id <> ?1")
      .bind(id),
    db
      .prepare(
  ...
  return Response.json({
    snapshot: { id, status: "approved", approvedBy, approvedAt: now },
