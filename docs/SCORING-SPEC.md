...
| SS-10 | Supply the permanent PRC Player ID crosswalk and scoring-source operating approvals | `PARTIALLY CLOSED`: the 413-player v1 crosswalk, aliases, 1,196 source-ID mappings, zero-open review queue, and 8,323-check validator are approved. Written FantasyPros API-use approval was received on 2026-07-20; its protected-key, manual-review integration preserves CSV fallback and reproducible snapshots. FantasyCalc permission plus the remaining archival, correction, coverage, and fallback approvals remain open. |
